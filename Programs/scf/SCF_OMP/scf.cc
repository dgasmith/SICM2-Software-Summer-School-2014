...
printf("\n OMP SCF code utilizing %d threads \n",omp_get_num_threads()); 
  ...
  try {
    ...
    auto enuc = 0.0;
    #pragma omp parallel for reduction(+:enuc) 
    for (auto i = 0; i < atoms.size(); i++)
    ...
    do {
      ...
      // compute HF energy
      ehf = 0.0;
     #pragma omp parallel for reduction(+:ehf) 
...
#pragma omp parallel default(none) shared(D,shells,Gtotal,n) 
{
...
#pragma omp for collapse(4)
  for(int s1=0; s1<shells.size(); s1++){
    for(int s2=0; s2<shells.size(); s2++){
	for(int s3=0; s3<shells.size(); s3++){
	//const int s4_max = (s1 == s3) ? s2 : s3;
		for(int s4=0; s4<shells.size(); s4++){
		
		if(s2>s1){continue;}
		if(s3>s1){continue;}
		if(s1==s3){
			if(s4>s2){continue;}}
		else{if(s4>s3){continue;}}
